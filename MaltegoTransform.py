...
class MaltegoEntity(object):
    ...
    def addAdditionalFields(self, fieldName=None, displayName=None, matchingRule=False, value=None):
        self.additionalFields.append(
            [sanitise(fieldName), sanitise(displayName), matchingRule, sanitise(value)])
    ...
    def setLinkColor(self, color):
        self.addAdditionalFields(
            'link#maltego.link.color', 'LinkColor', '', color)

    def setLinkStyle(self, style):
        self.addAdditionalFields(
            'link#maltego.link.style', 'LinkStyle', '', style)

    def setLinkThickness(self, thick):
        self.addAdditionalFields(
            'link#maltego.link.thickness', 'Thickness', '', str(thick))
            ...
            for i in range(len(self.additionalFields)):
                if (str(self.additionalFields[i][2]) != "strict"):
                    ...
                    print("<Field MatchingRule=\"" + str(self.additionalFields[i][2]) + "\" Name=\"" + str(
                        self.additionalFields[i][0]) + "\" DisplayName=\"" + str(self.additionalFields[i][1]) + "\">" + str(self.additionalFields[i][3]) + "</Field>")
            print("</AdditionalFields>")
